...
const FormEditProduct = () => {
  ...
  useEffect(() => {
    const getProductById = async () => {
      try {
        ...
        await axios.get(`http://localhost:5000/products/${id}`, data);
        navigate("/products");
      } catch (error) {
        if (error.response) {
  ...
  const updateProduct = async (e) => {
    ...
    try {
      ...
      await Axios.patch("http://localhost:5000/products/edit", data);
      navigate("/products");
    } catch (error) {
      if (error.response) {
        // setMsg(error.response.data?.msg);
  ...
  return (
    <div>
      ...
      <h2 className="subtitle">Edit Data Product</h2>
